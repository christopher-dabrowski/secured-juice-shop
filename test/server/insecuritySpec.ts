...
describe("insecurity", () => {
  describe("cutOffPoisonNullByte", () => {
    it("returns string unchanged if it contains no null byte", () => {
      expect(security.cutOffPoisonNullByte("file.exe.pdf")).to.equal(
        "file.exe.pdf"
      );
    });

    it("returns string up to null byte", () => {
      expect(security.cutOffPoisonNullByte("file.exe%00.pdf")).to.equal(
        "file.exe"
      );
    });
  });
  ...
  describe("userEmailFrom", () => {
    it('returns content of "x-user-email" header if present', () => {
      expect(
        security.userEmailFrom({
          headers: { "x-user-email": "test@bla.blubb" },
        })
      ).to.equal("test@bla.blubb");
    });
  ...
  describe("generateCoupon", () => {
    ...
    it("does not encode day of month or time into coupon code", () => {
      ...
      expect(coupon).to.equal(
        security.generateCoupon(10, new Date("December 01, 1999 01:00:00"))
      );
      expect(coupon).to.equal(
        security.generateCoupon(10, new Date("December 02, 1999"))
      );
      expect(coupon).to.equal(
        security.generateCoupon(10, new Date("December 31, 1999 23:59:59"))
      );
    });
  });
  ...
  describe("discountFromCoupon", () => {
    ...
    it("returns undefined for coupon code not according to expected pattern", () => {
      expect(security.discountFromCoupon(z85.encode("Test"))).to.equal(
        undefined
      );
      expect(security.discountFromCoupon(z85.encode("XXX00-10"))).to.equal(
        undefined
      );
      expect(security.discountFromCoupon(z85.encode("DEC18-999"))).to.equal(
        undefined
      );
      expect(security.discountFromCoupon(z85.encode("DEC18-1"))).to.equal(
        undefined
      );
      expect(security.discountFromCoupon(z85.encode("DEC2018-10"))).to.equal(
        undefined
      );
    });

    it("returns undefined for expired coupon code", () => {
      expect(security.discountFromCoupon(z85.encode("SEP14-50"))).to.equal(
        undefined
      );
    });

    it("returns discount from valid coupon code", () => {
      expect(security.discountFromCoupon(security.generateCoupon(10))).to.equal(
        10
      );
      expect(security.discountFromCoupon(security.generateCoupon(99))).to.equal(
        99
      );
    });
  });
  ...
  describe("authenticatedUsers", () => {
    it("returns user by associated token", () => {
      security.authenticatedUsers.put("11111", {
        data: { id: 1 } as unknown as UserModel,
      });

      expect(security.authenticatedUsers.get("11111")).to.deep.equal({
        data: { id: 1 },
      });
    });
    ...
    it("returns token by associated user", () => {
      ...
      expect(
        security.authenticatedUsers.tokenOf({ id: 1 } as unknown as UserModel)
      ).to.equal("11111");
    });
    ...
    it("returns user by associated token from request", () => {
      ...
      ).to.deep.equal({ data: { id: 1 } });
    });

    it("returns undefined if no token is present in request", () => {
      expect(
        security.authenticatedUsers.from({ headers: {} } as unknown as Request)
      ).to.equal(undefined);
      expect(
        security.authenticatedUsers.from({} as unknown as Request)
      ).to.equal(undefined);
    });
  });
  ...
  describe("sanitizeHtml", () => {
    ...
    it("returns input unchanged for plain text input", () => {
      expect(security.sanitizeHtml("This application is horrible!")).to.equal(
    ...
    it("returns input unchanged for HTML input with only harmless text formatting", () => {
      expect(
        security.sanitizeHtml(
          "<strong>This</strong> application <em>is horrible</em>!"
        )
      ).to.equal("<strong>This</strong> application <em>is horrible</em>!");
    });

    it("returns input unchanged for HTML input with only harmless links", () => {
      expect(
        security.sanitizeHtml(
          '<a href="bla.blubb">Please see here for details!</a>'
        )
      ).to.equal('<a href="bla.blubb">Please see here for details!</a>');
    });

    it("removes all Javascript from HTML input", () => {
      expect(
        security.sanitizeHtml(
          'Sani<script>alert("ScriptXSS")</script>tizedScript'
        )
      ).to.equal("SanitizedScript");
      expect(
        security.sanitizeHtml('Sani<img src="alert("ImageXSS")"/>tizedImage')
      ).to.equal("SanitizedImage");
      expect(
        security.sanitizeHtml(
          'Sani<iframe src="alert("IFrameXSS")"></iframe>tizedIFrame'
        )
      ).to.equal("SanitizedIFrame");
    });
    ...
    it("can be bypassed by exploiting lack of recursive sanitization", () => {
      ...
      ).to.equal('<iframe src="javascript:alert(`xss`)">');
    });
  });
  ...
  describe("sanitizeLegacy", () => {
    ...
    it("can be bypassed to allow working HTML payload to be returned", () => {
      expect(
        security.sanitizeLegacy("<<a|ascript>alert(`xss`)</script>")
  ...
  describe("sanitizeSecure", () => {
    ...
    it("returns input unchanged for plain text input", () => {
      expect(security.sanitizeSecure("This application is horrible!")).to.equal(
        "This application is horrible!"
      );
    });
    ...
    it("removes all Javascript from HTML input", () => {
      ...
      expect(
        security.sanitizeSecure('Sani<img src="alert("ImageXSS")"/>tizedImage')
      ).to.equal("SanitizedImage");
      expect(
        security.sanitizeSecure(
          'Sani<iframe src="alert("IFrameXSS")"></iframe>tizedIFrame'
    ...
    it("cannot be bypassed by exploiting lack of recursive sanitization", () => {
      expect(
        security.sanitizeSecure(
          'Bla<<script>Foo</script>iframe src="javascript:alert(`xss`)">Blubb'
        )
      ).to.equal("BlaBlubb");
    });
  });
  ...
  describe("hash", () => {
    it("returns MD5 hash for any input string", () => {
      expect(security.hash("F7eqgNr8R2tx5Jf@q7xT^E5!h@g%Sy")).to.equal(
        "7654a09d5fbcef1ff733a7ed69d7b877"
      );
  ...
  describe("hmac", () => {
    it('returns SHA-256 HMAC with "pa4qacea4VK9t9nGv7yZtwmj" as salt any input string', () => {
      expect(security.hmac("F7eqgNr8R2tx5Jf@q7xT^E5!h@g%Sy")).to.equal(
        "567147a82dfe88b40800df72b9e8d85e32735afda8df30990688c60c0589d3b2"
      );
      expect(security.hmac("password")).to.equal(
        "da28fc4354f4a458508a461fbae364720c4249c27f10fccf68317fc4bf6531ed"
      );
      expect(security.hmac("")).to.equal(
        "f052179ec5894a2e79befa8060cfcb517f1e14f7f6222af854377b6481ae953e"
      );
    });
  });
});
