    ...
    return frisby
      .post(REST_URL + "/user/login", {
        ...
        body: {
          ...
          password: "F7eqgNr8R2tx5Jf@q7xT^E5!h@g%Sy",
        },
      })
      .expect("status", 200)
      .expect("header", "content-type", /application\/json/)
  ...
  it('POST login with non-existing email "acc0unt4nt@juice-sh.op" via UNION SELECT injection attack', () => {
    return frisby
      .post(REST_URL + "/user/login", {
        header: jsonHeader,
        body: {
          email: `' UNION SELECT * FROM (SELECT 15 as 'id', '' as 'username', 'acc0unt4nt@${config.get<string>(
            "application.domain"
          )}' as 'email', '12345' as 'password', 'accounting' as 'role', '' as deluxeToken, '1.2.3.4' as 'lastLoginIp' , '/assets/public/images/uploads/default.svg' as 'profileImage', '' as 'totpSecret', 1 as 'isActive', '1999-08-16 14:14:41.644 +00:00' as 'createdAt', '1999-08-16 14:33:41.930 +00:00' as 'updatedAt', null as 'deletedAt')--`,
          password: undefined,
        },
      })
      .expect("status", 200)
...
describe("/rest/saveLoginIp", () => {
  ...
  xit("GET last login IP will be saved as remote IP when True-Client-IP is not present", () => {
    // FIXME Started to fail regularly on CI under Linux
    return frisby
      .post(REST_URL + "/user/login", {
        headers: jsonHeader,
        body: {
