...
describe("/#/contact", () => {
  ...
  describe('challenge "forgedFeedback"', () => {
    beforeEach(() => {
      cy.login({ email: "admin", password: "F7eqgNr8R2tx5Jf@q7xT^E5!h@g%Sy" });
      cy.visit("/#/contact");
      solveNextCaptcha();
    });
  ...
  describe('challenge "dlpPastebinDataLeak"', () => {
    it("should be possible to post dangerous ingredients of unsafe product as feedback", () => {
      cy.get("#rating").type("{rightarrow}{rightarrow}{rightarrow}");
      cy.task<ProductConfig>("GetPastebinLeakProduct").then(
        (pastebinLeakProduct: ProductConfig) => {
          cy.get("#comment").type(
            pastebinLeakProduct.keywordsForPastebinDataLeakChallenge
