...
export class LoginComponent implements OnInit {
      ...
      next: (config) => {
        if (config?.application?.googleOauth) {
          ...
          const authorizedRedirect =
            config.application.googleOauth.authorizedRedirects.find(
              (r) => r.uri === this.redirectUri
            );
          if (authorizedRedirect) {
            this.oauthUnavailable = false;
            this.redirectUri = authorizedRedirect.proxy
              ? authorizedRedirect.proxy
              : authorizedRedirect.uri;
          } else {
            this.oauthUnavailable = true;
            console.log(
              this.redirectUri +
                " is not an authorized redirect URI for this application."
            );
          }
        }
      },
    ...
    this.userService.login(this.user).subscribe({
      ...
      error: ({ error }) => {
        if (
          error.status &&
          error.data &&
          error.status === "totp_token_required"
        ) {
          localStorage.setItem("totp_tmp_token", error.data.tmpToken);
          this.ngZone.run(
            async () => await this.router.navigate(["/2fa/enter"])
          );
          return;
        }
  ...
  googleLogin() {
    const state = this.getRandomInt(0, 1000000000).toString();
