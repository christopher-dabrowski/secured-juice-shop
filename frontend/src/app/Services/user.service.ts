...
export class UserService {
  ...
  find(params?: any) {
    return this.http
      .get(this.hostServer + "/rest/user/authentication-details/", { params })
      .pipe(
  ...
  login(params: any) {
    ...
    return this.http.post(this.hostServer + "/rest/user/login", params).pipe(
      map((response: any) => response.authentication),
      catchError((err) => {
        throw err;
      })
    );
  ...
  resetPassword(params: any) {
    return this.http
      .post(this.hostServer + "/rest/user/reset-password", params)
      .pipe(
        map((response: any) => response.user),
        catchError((err) => {
          throw err;
  ...
  oauthLogin(accessToken: string, state: string) {
    const myState = window.sessionStorage.getItem("oauthState");
    if (state !== myState) throw new Error("Invalid OAuth state");
  ...
  deluxeStatus() {
    return this.http.get(this.hostServer + "/rest/deluxe-membership").pipe(
      map((response: any) => response.data),
      catchError((err) => {
        throw err;
      })
